...
-- Um UPSERT de playlist_tracks pode disparar o gatilho mesmo quando a faixa já
-- está vinculada à playlist. Nesse caso a posição nova deve ficar como
-- duplicada, sem tentar atribuir novamente o mesmo track_id ao histórico.
...
begin
  ...
  select prt.id into v_existing_item_id
    from public.playlist_request_tracks prt
   where prt.playlist_request_id = v_request_id
     and prt.track_id = new.track_id
   order by prt.captured_at, prt.id
   limit 1;
  ...
  if v_item_id is not null and v_existing_item_id is not null then
